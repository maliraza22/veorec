...
(async () => {
  ...
  try {
    ...
    ok(r1.relayed >= 1 && a1.enqueuedAt instanceof Date && (await q.has(a.id, 'probe')), 'the relay hands the row to the transport and stamps enqueued_at (other unstamped rows from earlier runs may ride along)');
    ...
    await app7.start();
    const j = await insert('email', 'j');
    ok(await until(async () => !!(await rowOf(j.id)).enqueuedAt, 3000) && (await q.has(j.id, 'email')), 'a default worker relays outbox rows into the transport where they wait for a consumer (email has no processor yet)');
    ok(app7.status().queues.sort().join() === 'ai,maintenance,media,render,stt' && (await rowOf(j.id)).status === 'queued', 'the default worker consumes maintenance/stt/ai/media/render (T-602/T-603/T-701/T-1202) and never the email queue');
    await app7.stop();
  } finally {
