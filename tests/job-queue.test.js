...
(async () => {
  ...
  console.log('\nD. Processor registry');
  {
    ...
    ok(shipped.types().sort().join() === 'ai_chapters,ai_summary,ai_title,cleanup,probe,transcode,transcribe,translate,upload_expiry,usage_sync' && shipped.queues().sort().join() === 'ai,maintenance,media,stt', 'the shipped registry carries the T-602 maintenance, T-603 stt/ai and T-701/T-702 probe+transcode processors (T-703+ add the rest of media)');
