...
(async () => {
  ...
  ok(/client\.view\(\{ visitorId: getVisitorId\(\) \}\)/.test(watch) && /client\.engagement\(\)/.test(watch) && /client\.react\(\{ emoji, t, name: commentName \}\)/.test(watch) && /client\.comment\(\{ text: commentText, name: commentName, t \}\)/.test(watch) && /client\.progressBeacon\(maxFrac/.test(watch) && !/\/api\/watch\/\$\{id\}\/(view|engagement|comment|react|progress)/.test(watch), 'engagement (views/comments/reactions/progress) goes through the data layer for BOTH sources (T-1001) — the page builds no engagement URL');
