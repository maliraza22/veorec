...
> **As implemented (T-1202/T-1203):** type `render` on the `render` queue (`TYPE_CONCURRENCY` 1 per process, 2 attempts, 15 min catalog timeout; the processor's own ffmpeg timeout is max(15 min, 4× the output duration)). Payload `{editSessionId, renderJobId, mode, outputRecordingId, timeline, durationSec, canvasFrom, sources}`. `worker/src/processors/render.js`: check-before-do (`render_jobs.done` → skipped), each clip resolved to its normalised MP4 else its immutable source, downloaded to scratch (disk-room check), probed; `worker/src/media/render.js` plans **stream copy** for single-source timelines (per-cut `-ss/-to -c copy` + concat demuxer, then ffprobe verification — a copy outside max(2 %, 250 ms × clips) is re-encoded) and a **filter-graph re-encode** for multi-source timelines on the base's canvas (1920 long-edge cap, even, letterbox pad, 48 kHz stereo AAC with `anullsrc` for silent clips, libx264 veryfast crf 23, +faststart, `-progress` → `result.progress` every 5 %). **copy:** the output is uploaded to `sources/{outputId}/source.mp4` and becomes the NEW recording's immutable, billed source; the render's reservation is reconciled to the real size (`storage_reservations.render_job_id`, retained bytes + one active video charged); a `probe` job is enqueued so the normal pipeline promotes it to ready. **overwrite:** the output is uploaded to `renders/{session}/{job}.mp4`, the `mp4/main` asset row is **re-pointed** at it (facts updated), stale `hls`/`captions_vtt`/`audio` rows are dropped (their objects are orphans for the weekly cleanup), the recording's duration/size and virtual edits (`trim_*`, `segments`) are reset; the source object is never touched (asserted byte-for-byte). Failures (`14` §8 taxonomy) → `render_jobs.failed` + the session back to `draft`; a copy's pre-created output recording is marked failed and its reservation released.

### `silence_detect` (spec `14` §7)
- In: `{recordingId, pad, minGap}`; dedupe `silence:{recordingId}`; queue `media`, 2 attempts, 20 min.
- Does: `ffmpeg -af silencedetect=noise=-30dB:d={minGap}` over the audio asset (else the MP4, else the source) → keep-ranges (padded, a silence touching the start/end is cut flush); no audio signal → the transcript-gap rule (the legacy `keepRangesFromTranscript`); applied as a **virtual** edit (`recordings.segments`, trims cleared) — instant, reversible, nothing rendered.
- Out: `result {segments, keptSeconds, removedSeconds, duration, method:'audio'|'transcript'}` read through `GET /api/v1/recordings/:id/remove-silences`. Terminal reasons: `recording_not_ready`, `source_missing`, `no_silence` (nothing to trim), `no_speech` (video-only without a transcript), `silence_ffmpeg_failed`.
