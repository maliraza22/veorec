...
### 5.1 As implemented (T-705)

`worker/src/media/hls.js` — `createHlsPackager({ ffmpegBin, prober })`: renditions **no taller than the source** from the table above (1080p@5M only when the source is ≥ 1080p; a smaller source gets one rendition at its own height), each rendition its own ffmpeg pass (libx264 veryfast high@4.1, maxrate/bufsize, GOP = 4 s, `sc_threshold 0`, aac 128k or `-an` for silent sources) writing **flat files inside the output directory** — `{name}_index.m3u8`, `{name}_init.mp4`, `{name}_seg_00001.m4s` — because ffmpeg resolves the fMP4 init-segment name against the process cwd (a `%v` placeholder is not expanded there) and because the key contract (`keys.hlsSegment`) forbids nested names; the master playlist is written by the worker from the probed variant facts (`BANDWIDTH` +10 %, `AVERAGE-BANDWIDTH`, `RESOLUTION`, `CODECS="avc1.PPCCLL,mp4a.40.2"`, `FRAME-RATE`). **Verification before publish**: every variant's init segment is ffprobed (streams, height, profile/level → codec string), every variant playlist is decoded end-to-end by ffmpeg (duration within 5 %), and the playlists are inspected (master lists every variant; each variant has `EXT-X-MAP`, ≥ 1 segment, `EXT-X-ENDLIST`, summed `EXTINF` within 2 % of the source). The `hls` processor (media queue, serialised per process, 3× source scratch guard) uploads the master to `keys.hlsMaster(recordingId, assetId)` and every other file to `keys.hlsSegment(recordingId, assetId, name)` with its content type (`application/vnd.apple.mpegurl`, `video/mp4`, `video/iso.segment`), then flips the single `hls` asset row `ready` (facts of the top rendition, `container='hls'`, not billed). It **never calls `maybe_mark_ready`** — HLS is an enhancement; check-before-do skips a ready asset whose master exists; `payload.force` repackages into the same row. **Playback verified** with hls.js 1.5 in the in-app Chromium: a 10-minute package (150 × 4 s segments) plays, seeks to 595 s and ends at 600 s with no fatal error; a 1440p package exposes three levels and switches 480p ↔ 1080p on demand with the decoded width following. **Not verified here:** Safari native HLS (no Safari on this machine).
