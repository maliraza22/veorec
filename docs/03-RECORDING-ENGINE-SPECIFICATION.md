...
`extension/machine.js` — plain UMD (`VeoRecMachine.createMachine({ effects, now, log, onTransition })`), **pure**: no DOM, no `chrome.*`, no MediaRecorder, no network. States, events and transitions are exactly §2 (plus the internal `finalizing`, `STOP_TIMEOUT` for the §3.6 guard, `CHUNK`/`TICK` for the §7–§8 pipeline, `TRACK_ENDED`/`MIC_LOST`/`WARNING` for §9, `UPLOAD_PROGRESS`). Every side effect is a named **effect** the host injects (`acquire`, `startCountdown`, `createSession`, `createUploadSession`, `startRecorder`, `pauseRecorder`/`resumeRecorder`, `stopRecorder`, `stopTracks`, `finalize`, `startUpload`, `complete`, `download`, `discard`, `cleanup`, `wakeLock`, `setTimer`, `publish`, `persist`, `clearProjection`, `warn`, `chunk`); effects are never awaited — they send events back; a missing effect is simply not requested; a throwing effect is logged and never breaks a transition. **Entry order of `recording`** is §3.4's: session row → upload session → recorder → clock → wake lock → projection; resume from `paused` does not re-run it. **Elapsed** is computed in exactly one place (`elapsedMs`, §3.5 formula) and the projection carries `startedAt`/`pausedTotal`/`pauseStartedAt`. **`stopping`** calls `stopRecorder` and nothing else, with a 10 s guard that forces `finalizing` if `onstop` never arrives; `finalizing` stops the tracks and releases the wake lock. **Limits (§8)**: duration and bytes enforced from the chunk clock with a re-arming backstop timer, a once-only 30 s / 90 % warning, `STOP(source:'limit'|'byte_limit')`, `clientDuration` clamped to the limit. **Disposer registry (§10)**: `register(id, ownerState, dispose)` returns an unregister closure; the owning state's exit disposes its own; `idle`/`cancelling`/terminal states `disposeAll()`; `leaks()` is the dev assertion. **Projection (§11)** on every transition with the legacy keys (`recording`, `recState`, `startTime`) mirrored. **Dispatch**: one event at a time on a queue, so an effect that sends synchronously is processed after the current transition; unknown or invalid-in-state events are ignored and logged, never thrown. `cancelling` sets `discarding` and its own `CHUNK` handler drops the pipeline — the `onstop=null` trick is gone. Tests: `tests/machine.test.js` (428 assertions — every state × every event, entry order, elapsed, limits, interruptions, disposers, projection, re-entrancy) with **100 % block coverage** measured by V8 precise coverage (`tests/coverage-machine.js`, both load paths). Since T-503 the recorder window is a renderer of this machine (§2.2).

### 2.2 As wired into the recorder window (T-503)

`extension/recorder.js` no longer holds recording state of its own: every button, every `SR_*` / `STOP_RECORDING` message and every browser callback becomes a `machine.send(...)`, and everything the user sees is `render(projection, state, ctx)` called from the `publish`/`warn` effects. The injected effects are the module's only side effects: `acquire` → `VeoRecCapture.createCaptureManager().acquire(config)` (`ACQUIRED {tracks, warnings}` / `ACQUIRE_FAILED {code}`, `18` §4 codes passed through), `startCountdown` (the plan-matched 3 s / 0 s countdown → `COUNTDOWN_DONE`), `createSession` (the T-402 local IndexedDB session), `createUploadSession` (the T-303 streaming uploader when `/api/client-config` says v1, otherwise a no-op), `startRecorder` (a real `MediaRecorder` from `recorderOptions()`; `dataavailable` → `CHUNK {size}` after the chunk is fed to the store and the uploader, `onstop` → `RECORDER_STOPPED`, `onerror` → `STOP(source:'recorder_error')`, plus `captureManager.watch()` forwarding `TRACK_ENDED`/`MIC_LOST`), `pauseRecorder`/`resumeRecorder`, `stopRecorder` (sends `RECORDER_STOPPED` itself when the recorder is already inactive), `stopTracks` (capture disposal + watcher off), `finalize` (blob assembly, duration fix, empty-blob → `UPLOAD_FAILED {code:'empty'}` with the local session kept), `startUpload` (`runUpload`: v1 finalize or the legacy multipart POST → `UPLOAD_COMPLETE` / `UPLOAD_FAILED {code, retryable}` with `plan_limit`, `unauthorized`, `timeout`, `network`, `http_N`), `complete` (watch tab, `lastRecording`, local session deleted only after the server's 200), `download` / `discard`, `cleanup` → `CLEANED`, `wakeLock`, `setTimer` (the §8 backstop), `persist`, `warn`, `clearProjection`, `publish`. The streaming uploader's own `ceiling-warning` / `ceiling-reached` events become `WARNING {code:'byte_limit_near'}` / `STOP(source:'byte_limit')` — the byte limit has exactly one owner. Warning texts are rendered from `VeoRecCapture.WARNING_TEXT[code]` (no duplicated strings; the overlay gets the same text via `SR_OVERLAY_WARN`). **Mic-denied UI (§3.2):** `permission_denied` with `code:'mic_denied'` renders three buttons — *Record without mic* (`RETRY {config:{audio:false}}`), *Fix permission* (opens the mic settings page, then `RETRY`), *Cancel* (`DISMISS`); any other code renders the `18` §4 message with Retry / Cancel. **`upload_failed`** renders Retry (`RETRY_UPLOAD`, only when `retryable`), Save locally (`DOWNLOAD_FALLBACK`), Discard; the local session is kept so a closed window still recovers on the next launch (`05` §6). A `beforeunload` guard warns while `recording`/`paused`/`stopping`/`finalizing`/`uploading`. The launch sequence is unchanged: options → plan limit → recovery scan (T-403) → quota pre-flight (T-307, §3.0) → `START {config, durationLimitSec}`. Removed with the rewrite: the module-level `isRecording`/`isPaused`/`pausedAccum` booleans, the synchronous stop (tracks were stopped before the recorder flushed), the `onstop = null` cancel trick, and the hand-rolled limit timer — all now the machine's. Tests: `tests/recorder-ui.test.js` (110 — the real `recorder.js` loaded in a vm with a DOM/chrome/MediaRecorder stub set: every effect's send, the mic-denied three-way choice, upload-failure mapping, the projection/legacy-key mirror, message routing, the unload guard) and `tests/recorder-wiring.test.js` (44). Real-browser E2E with only `chrome.*` and the picker stubbed: countdown → recording → pause/resume → stop → completed against the local v1 API, plan-limit auto-stop, cancel with no disposer leaks, mic-denied choice.
...
### 4.1 As implemented (T-502)

`extension/capture.js` — UMD `VeoRecCapture.createCaptureManager({ mediaDevices, AudioContext, isTypeSupported, sleep, log })`, every browser dependency injectable (browser globals by default). **`constraintsFor(config)`** is the table above verbatim: tab mode keeps the REQUIRED legacy `mandatory` shape with tab audio; screen/window pass `displaySurface` + `systemAudio:'include'`; camera-only is 720p with no audio; quality → 1920×1080 / 1280×720 / 854×480 at 4 / 2.5 / 1 Mbps. **`acquire(config)`** acquires in order (display or tab or camera, then the mic), builds the mixer (§5), computes the warnings and returns `{ tracks:{video,audio}, streams, warnings, warningText, mixer, dispose }`; on any failure every acquired track is stopped first, then a `CaptureError` with a `18` §4 code is thrown: `permission_dismissed` (picker cancelled) / `permission_denied` / `no_device` / `tab_capture_failed` / `constraint_failed` (after **one automatic retry at the next lower quality**) / `device_busy` / `capture_failed`. **Mic denial policy (§3.2):** a failed mic while `audio:true` throws `mic_denied` — never a silent mic-less take; "Record without mic" is simply `acquire({...config, audio:false})`. **`mix()`** (§5): one AudioContext, a gain node per source (+0 dB), the tab loopback to the speakers only in tab mode, a mic stream counted only if it carries an audio track; the suspended-context safeguard `resume()` → wait ≤ 1.5 s for `running` → otherwise `suspended:true`, which `acquire` surfaces as the `audio_context_suspended` warning instead of proceeding silently; an `onstatechange` re-resume; `dropMic()` removes a lost mic from the mix. **Warnings** carry the legacy audience-facing texts verbatim (`no_audio_at_all`, `tab_audio_missing`, `no_system_audio`) plus `audio_context_suspended`, `bubble_not_captured`, `mic_lost`, `tab_audio_lost`. **`recorderOptions()`** (§6): `vp9,opus` → `vp8,opus` → `webm` via `isTypeSupported`, the quality bitrate, and `audioBitsPerSecond: 128 000` explicitly. **`watch()`** (§9): `ended` on the video / mic / tab-audio tracks → `TRACK_ENDED {kind}` (a lost mic is dropped from the mix, recording continues), `devicechange` with the active mic gone → `MIC_LOST` (no auto-switch); returns a disposer. No canvas compositing (§13). Not wired yet: T-503 makes it the machine's `acquire` effect.
...
**As implemented (T-503):** the `publish` effect writes `recSession` (the projection above, plus `recording: boolean` and `sessionId`) and, in the same `chrome.storage.local.set`, the mirrored legacy keys `recording`, `startTime` and `recState = { recording, startTime, paused, pausedAccum, pauseStartedAt }` (or `{ recording: false }`) — so `overlay.js` (`recState` timer), `popup.js` (`recording`/`startTime`) and `background.js` (`recState.recording` for overlay re-injection) keep working unchanged until Phase 14. The projection is written on every transition; the window's own 500 ms tick drives only the visible timer, not storage.
