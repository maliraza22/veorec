...
- Rate limits (Redis, per user or IP): auth 10/15min·IP, signup 15/h·IP, forgot 5/h·IP, comments/reactions 30/min·IP, views/progress 60/min·IP, uploads 10 sessions/h·user, AI endpoints 10/h·user. 429 + `Retry-After`. *(T-1304, as implemented — `api/src/rate-limit.js`: fixed windows on ONE Redis budget across API instances (`createRedisRateStore`, an atomic INCR + PEXPIRE + PTTL script, keys `<prefix>rl:<name>:<scope>:<caller>`) whenever `REDIS_URL` is set, else the per-process window; watch / view / engage / lead limits **open-fail** and unlock / upload-session / AI-trigger limits **closed-fail** (the per-process fallback still denies) when Redis is unreachable (`17` §5). Upload-session creation is 10/h per user (`rateLimits.sessions`), AI triggers — transcribe, translate, title, summary, chapters, reprocess — 10/h per user (`rateLimits.ai`); reads are never limited. The legacy auth limiters (`server/ratelimit.js`) stay per-process until the auth routes move to v1 (T-1302).)*
...
| GET `/auth/sessions` | ⚿ | – | `{items:[{id,client,userAgent,ip,createdAt,lastUsedAt,expiresAt,current}], current}` | *(T-1302)* the caller's live sessions |
| DELETE `/auth/sessions/:id` | ⚿ | – | `{ok}` | *(T-1302)* revoke another device (own sessions only; 404 otherwise) |

*(T-1302, as implemented — `api/src/auth.router.js` on `/api/v1/auth`, `api/src/sessions.js`: tokens are opaque `vs_` + 256 bits (base64url), only sha256(token) is stored (`sessions.token_hash`), 30-day rolling expiry (touched at most every 5 min), `client` web/extension from `x-veorec-client`. Sign-up: 201, password ≥ 8, bcrypt cost 12 (the server injects `passwords`), `409 email_taken`. Login/Google share one generic `401 invalid_credentials` / `invalid_google_credential` (Google: injected `googleVerify`, `aud` and the string-"false" `email_verified` guards, link by e-mail, create without a password). Logout revokes THIS session; password change revokes every OTHER session; reset (token hash + 1 h, single-use) revokes ALL and issues a fresh one; forgot always answers `{ok}` and mails only password accounts through the injected sender. Every error is the nested contract; auth routes are rate-limited per IP on the shared store, closed-fail (`login 10/15 min`, `signup 15/h`, `forgot 5/h`, `reset 10/15 min`). The user body is the legacy `publicUser` shape (`id` = the LEGACY id, plus `pgId`) with entitlements resolved from PostgreSQL (T-1303). **Bridge compat during the migration window:** `server/auth.js#requireAuth` accepts a session token on EVERY route (`configureSessionAuth` → `createSessionResolver`), setting the same `req.userId` (legacy id) the legacy routes read plus `req.pgUserId`/`req.sessionId`; the legacy JWT keeps working on the v1 routes; accounts created on v1 also exist in the legacy store (the injected `accounts` adapter) so ids stay `usr_<legacyId>`. Gate: `V1_AUTH` (exact `'true'`, only with `V1_UPLOAD_API`) as the `auth` block of the PUBLIC client-config (a visitor has no token), KPI `auth_decision`.)*
...
| POST `/watch/:id/view` | optional | `{visitorId?}` → `{views}`; upserts `view_sessions` (unique viewer_key; owner ⇒ `is_owner`, not counted); naturally idempotent *(T-1001 ✅ `api/src/engagement.router.js`: key = signed-in user → `visitorId` → salted **daily** IP hash (the IP itself is never stored); `{views, self?}`; one `analytics_events` `view` per non-owner call; 60/min·IP)* |
| POST `/watch/:id/progress` | optional | `{pct}` (0–1, sendBeacon) → 204; upsert max_progress *(T-1001 ✅: `{pct, visitorId?}` — a beacon carries no headers, so the anonymous key travels in the body; monotonic `greatest`, `completed` at ≥ 0.9; garbage or an unknown session is a silent 204)* |
| GET `/watch/:id/engagement` | optional | `{views,reactions,comments}` *(T-1001 ✅: comments `{id,name,text,t,at,parentId,verified,removed}` oldest first (removed ones omitted), reactions `{id,emoji,t,at,name,verified}`; `at` in epoch ms — the shapes the page already rendered)* |
| POST `/watch/:id/comment` | optional | `{text,name?,t?,parentId?}` → `Comment`; 403 if audience.comments=false; rate-limited *(T-1001 ✅: `201`; body ≤ 2000; a signed-in caller's name is the ACCOUNT name and `userId` is set (the form name is ignored), an anonymous name is display-only and `verified:false`; one reply level — a reply to a reply is `400 comment_depth`; `403 audience_disabled` for viewers when comments are off (the owner may still comment); 30/min·IP. `DELETE /watch/:id/comments/:commentId` — the owner/admin or the signed-in author soft-deletes; others `403 forbidden`)* |
| POST `/watch/:id/react` | optional | `{emoji,t?,name?}` → `{reactions}`; 403 if disabled; rate-limited *(T-1001 ✅: `201`; emoji ≤ 8 characters; appended events, never a tally; the same identity and gate rules as comments; 30/min·IP shared with comments)* |
