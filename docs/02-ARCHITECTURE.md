...
### 2.7.5f Owner analytics + paywall facts on v1 (delivered by T-1002 / T-1003)

`api/src/analytics.router.js` (per-recording analytics and the overview, Pro-gated, from `view_sessions` / `comments` / `reactions` / `leads` / `analytics_events` through the repositories) and `api/src/paywall.js` (the one helper every router uses when it answers `feature_locked` or a plan limit: the response is unchanged, and one `analytics_events` `paywall_hit` with the legacy trigger name is recorded). The dashboard's analytics modal reads a v1 recording from `/api/v1/recordings/:id/analytics`.
...
### 2.7.5c The library on v1 (delivered by T-803)

`client/src/lib/libraryApi.mjs` + `client/src/hooks/useLibraryClient.js`: the dashboard, folders page and notifications bell read the server's `library` decision once per page and bind a client to it — `/api/v1/recordings` (paginated, live + archived), `/api/v1/folders`, `/api/v1/notifications` on the v1 path, the untouched legacy routes otherwise. Every row is normalised to the card shape and tagged with its `source`; writes (rename, meta, delete) follow the row's own source, so a legacy row the backfill has not reached keeps writing to the legacy API even on the v1 page. A v1 card never builds a Cloudinary URL: thumbnails are signed storage URLs minted by the API, and the only Cloudinary reference on the v1 path is the server-side poster READ fallback from `legacy.media_map`. Server side: `api/src/folders.router.js`, `api/src/notifications.router.js`, the batched media resolution in the list handler, and the `/watch/:id/media` legacy fallback.
