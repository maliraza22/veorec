...
- **Acceptance:** server restart mid-transcription → job completes on worker restart (test); cron.js deleted. *(T-602: cron.js schedules nothing v1 any more — usage_sync/upload_expiry/cleanup/storage verification run in the worker as repeatables — but it is NOT deleted: `subscription_sync` and the legacy usage/storage jobs read and write the JSON stores that only the API process may write, so they stay there until Phase 9–13 move billing/usage into PostgreSQL; the file goes in Phase 14 with the stores. `10` §3.)* *(T-603: `stt.transcribe`/`stt.translate`/`ai.*` run in the worker and the v1 API answers 202; the restart-safety test SIGKILLs a worker mid-transcription and the job completes on the surviving worker. The legacy synchronous `/api/recordings/:id/transcribe` stays for legacy recordings until the Phase 8 read cutover — no v1 recording is ever transcribed in the API process.)*
...
- **Backfill:** re-process legacy recordings from R2 sources (throttled queue fill), producing MP4+poster for all; Cloudinary URLs remain a read fallback until each recording has assets. *(T-803: the fallback is implemented — the v1 list's thumbnail and `GET /watch/:id/media` read `legacy.media_map.legacy_url` only when a recording has no v1 image / playable asset, and stop the moment one lands. It is a read of a stored URL, never a Cloudinary API call or write.)* *(T-706: `db/src/maintenance/pipeline-backfill.js` — `pipelineStats` (active / with-source / complete = ready MP4 + ready poster / pending / failed / rejected / probe-in-flight, completion %) and `queueFill` (dry run by default; `apply` (re)queues `probe:{id}` for every recording with a ready source but no MP4 + poster, oldest first, `limit` per run, in-flight recordings skipped, settled probe rows requeued with attempts reset, `includeFailed` retries `probe_invalid`/`transcode_failed`). Operators run `node db/src/cli/pipeline-backfill.js stats|fill [--apply --limit=n --include-failed]` repeatedly (or the admin trigger); the worker fleet does the processing through the normal fan-out. The dashboard numbers are `GET /api/v1/admin/pipeline`. The 100 % acceptance is measured in production against real legacy sources — pending until the T-204 backfill has copied them and a worker fleet runs.)*
