...
**T-106 · Reconciliation safety net** — Not merely a nightly report: the mechanism that *proves* PostgreSQL stays convergent with the legacy system throughout the migration, and the gate that must be clean before authority ever moves. Detects, at minimum: legacy records missing in PostgreSQL · PostgreSQL records missing in legacy (**including unmirrored deletes**) · field drift on important columns · outstanding failed dual-writes · orphaned child records · mapping inconsistencies (bad id derivation, ownership mismatch, duplicates) · anything that cannot be safely reconciled automatically. **Detection first: `--report` is the default and performs zero writes; repair requires an explicit `--repair`, and no nightly run may ever delete PostgreSQL data.** Deps: T-105. Accept: every detection class exercised by tests; nightly report runs read-only; diffs trending to zero before any read cutover.
