...
- *(T-1302, as implemented: `api/src/sessions.js` + `api/src/auth.router.js` — see `08` §2. Opaque `vs_…` tokens, sha256 at rest, rolling 30 days, revocation on logout (self), password change (all others), reset (all), `DELETE /auth/sessions/:id` (another device). The legacy JWT is still issued by the legacy routes until the auth gate flips per deployment (`V1_AUTH`), and remains accepted for 30 days after — it cannot be revoked, which is exactly why it is being replaced. bcrypt cost 12 and the 8-character minimum apply to every v1 sign-up, password change and reset; reset tokens are stored hashed with a 1 h expiry; the Google `tokeninfo` audience and `email_verified` guards are ported with the string-"false" gotcha and unit-tested against a stubbed verifier.)*
